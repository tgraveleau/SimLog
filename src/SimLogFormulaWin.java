...
public class SimLogFormulaWin extends JDialog
implements ActionListener {
	...
	/**
	 *  constructor 1
	 *  we create a textarea for the user to enter a formula that
	...
	/**
	 *  constructor 2
	 *  we create an arithmatic representation of a part of a circuit
	 *  starting at the gate given as a parameter
	 *
	 *  @param parent parent Frame
	 *  @param gate SimLogGate
	 */
	...
	public boolean isOk(String s) {
		...
		if (anasynt.isCorrect()) {
			...
			if (formula.isCorrect()) return true;
		}
		return false;
	}
		...
		if (e.getSource()==bOk) {
			...
			if (mode==ENTER_FORMULA) {
				if (isOk(s)) {
					state = true;
					dispose();
				} else appli.windowWarning("Formula is not correct");
			} else {
				dispose();
			}
	...
	private String buildFormulaFromGate(SimLogGate gate) {
		...
		case SimLogGate.XOR_GATE:
			...
			if (link!=null) s1=buildFormulaFromGate(link.getOutputGate());
			else s1="?";
			link=gate.getInputLink(1);
			if (link!=null) s2=buildFormulaFromGate(link.getOutputGate());
			else s2="?";
			break;
		...
		case SimLogGate.LED_GATE:
			link=gate.getInputLink(0);
			if (link!=null) return buildFormulaFromGate(link.getOutputGate());
		}
		switch(gate.getType()) {
		case SimLogGate.AND_GATE:
			s=new String("("+s1+"."+s2+")");
