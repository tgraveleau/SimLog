...
public class SimLogCircuit {
		 ...
		 *  @param x coordinate
		 *  @param y coordinate
		 *  @return index of gate in vector listOfgates or -1 if no gate
		 *           is found
		 */
		 ...
		 *  @param x coordinate
		 *  @param y coordinate
		 *  @return gate in vector listOfgates or null if no gate
		 *           is found
		 */
		 ...
		 *  @return index of gate in vector listOfgates that intersects
     *          with reference gate or -1 if no gate is found
		 */
		...
		public int getIntersectedGateIndex( SimLogGate g ) {
				...
				for (int i = 0; i < listOfGates.size(); i++) {
						gate = (SimLogGate) listOfGates.elementAt(i);
						if (g != gate) {
								if (gate.intersects( g.x, g.y )) return i;
						}
				}
				return -1;
		}
		 ...
		 *
		 *  @param g reference gate
		 *  @return gate in vector listOfgates that intersects
     *          with reference gate or null if no gate is found
		 */
		...
		public SimLogGate getIntersectedGate( SimLogGate g ) {
				...
				for (int i = 0; i < listOfGates.size(); i++) {
						gate = (SimLogGate) listOfGates.elementAt(i);
						if (g != gate) {
								if (gate.intersects( g.x, g.y )) return gate;
						}
				}
				return null;
		}
     ...
     *         position
		...
		public SimLogGate getIntersectedGate( int x, int y ) {
				...
				for (int i = 0; i < listOfGates.size(); i++) {
						gate = (SimLogGate) listOfGates.elementAt(i);
						if (gate.intersects( x, y )) return gate;
				}
				return null;
		}
		...
		public int getGateIndex( SimLogGate g ) {
				for (int i = 0; i < listOfGates.size(); i++) {
						if (g == (SimLogGate) listOfGates.elementAt(i)) return i;
				}
				return -1;
		}
		 ...
		 *
		 *  @param n index of gate to remove in vector listOfGates
		 */
		...
		public void removeGate( SimLogGate g ) {
			...
			if (g.getType() == SimLogGate.SWITCH_GATE) {
					if (name != null) {
						tabAvailSwitchNames[ ((int) name.charAt(0))-65 ] = true;
					}
		...
		/**
		 *  remove gate in given area. this method is used when deleting
  	 *  gates with a pressed, drag and realeased event. Gates are
		 *  removed only if they are inside the defined area
		 *
		 *  @param r area of selection where gates will be removed
		 */
		...
		/**
		 *  find a name for a switch. The name is given by a letter between
		 *  A to Z
		 *
		...
		/**
		 *  find a name for a LED. The name is given by "L" followed by a
		 *  number between 1 to 26
		 *
		 *  @return name of LED
		 */
		 ...
		 *  by a "G" followed by a number which is incremented.
		 *
		 *  @return name of gate
		 */
		 ...
		 *  rename a gate
		 *
		 *	@param gate gate to rename
		 *  @param name new name of the gate
		 */
		...
		/**
		 *  try to find if a new name to be assigned to a gate is not already
		 *  in use
		 *
		 *	@param gate gate that is being renamed
		 *  @param s new name
		 ...
		 *
		 *  @param out gate from which the link comes from (output gate)
		 *  @param in  gate that receives the link (input gate)
		...
		public int addLink( SimLogGate out, SimLogGate in ) {
				...
				if (out.getType() == SimLogGate.LED_GATE) return -3;
				if (in.getType() == SimLogGate.SWITCH_GATE) return -4;
				if ((port = in.getAvailablePort()) == -1) return -6;
				...
				if (out.checkInput() && out.checkOutput()) {
					if (out.getState() == SimLogGate.STATE_FOCUS) out.setNormalState();
				}
				if (in.checkInput() && in.checkOutput()) {
					if (in.getState() == SimLogGate.STATE_FOCUS) in.setNormalState();
				}
				modified = true;
				return 0;
		}
     ...
     */
		 ...
		 *  @param st type of gate
		 *  @param sn name of gate
		 */
		...
		/**
		 *  create and add a link that is read from a file on disk.
		 *  Parameters are given as strings.
		 *
		 *  @param s1 index of output gate
		 *  @param s2 index of input gate
		 *  @param s3 port number of input gate
		 */
	...
	public int getNbrLED() {
		...
		SimLogGate findGateByName(String s) {
			int i, size;
			SimLogGate gate;
			...
			for (i=0;i<size;i++) {
				...
				if (gate.getName()!=null) {
				  if (s.equals(gate.getName())) return gate;
				}
			}
			return null;
		...
		private int evaluate(SimLogGate gate, int depth) {
			...
			n=listOfGates.size();
			for (i=0;i<n;i++) {
				if (gate==(SimLogGate)listOfGates.elementAt(i)) {n=i; break;}
			}

			// set relative x position
			if (depth>gate.x) gate.x=depth;
			if (depth>reorganizeX) reorganizeX=depth;
			++depth;
		...
		private void setPositions(int height,SimLogGate gate,int ypos) {
			...
			if ((gate.x!=-1) && (gate.y!=-1)) return ;
			gate.x=(reorganizeX-gate.x)*100;
			gate.y=height*70+ypos;
			if (gate.y>reorganizeY) reorganizeY=gate.y;
