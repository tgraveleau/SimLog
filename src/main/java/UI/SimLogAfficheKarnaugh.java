...
import Moteur.SimLogKarnaugh;
import Moteur.SimLogTableCorres;
...
public class SimLogAfficheKarnaugh extends JFrame {
	...
	// matrice contenant les l�gendes
	...
	// Tableau de Booleens pour Savoir les Masques � Afficher
	...
	// Renvoie la Formule Simplifi�e pour Chaque Bit
	// 1 -> La Variable
	// 0 -> Sa N�gation
	...
	public int[] CreerMonome(int[][] TabCases, int NbLigCaseMask) {
		...
		// Initialisation de la Table R�sultat avec la Premi�re Ligne
	...
	public void paint(Graphics g) {
		...
		// On Affiche la L�gende Horizontale
		...
		// On Affiche la L�gende Verticale
		...
		// On Affiche les Diff�rents Regroupements
		...
		// Parcours de Chacun des Masques pour colorier les masques coch�s
		...
		for (int i = 0; i < Karnaugh.getNbMask(); i++) {
			...
			if (CocheMask[i]) {
				...
				int[][] TabCases = Karnaugh.IniCaseMask(Mask, NbLigCaseMask,
						vide);
				...
				for (int h = 0; h < NbLigCaseMask; h++) {
					...
					g.fillOval((colonne + 1) * xZone + 2, (ligne + 2) * yZone
							+ 2, xZone - 3, yZone - 3);
		...
		// parcours de tous les masques pour afficher les "1" et
		// l'initialisation de la liste si besoin
		...
		for (int i = 0; i < Karnaugh.getNbMask(); i++) {
			...
			for (int h = 0; h < NbLigCaseMask; h++) {
				...
				// on met des 1 dans les cases dont les masques n'ont pas �t�
				// selectionn�s

				g.setColor(Color.black);
				g.drawString("1", (colonne + 1) * xZone + (xZone / 2) - 3,
						(ligne + 2) * yZone + (yZone / 2) + 5);
	...
	// initialise al�atoirement les couleurs
	...
	public void FixeColors(int SizeVect) {
		...
		if (SizeVect > 1)
			TabColors[1] = Color.red;
		if (SizeVect > 2)
			TabColors[2] = Color.green;
		if (SizeVect > 3)
			TabColors[3] = Color.magenta;
		if (SizeVect > 4)
			TabColors[4] = Color.orange;
		if (SizeVect > 5)
			TabColors[5] = Color.cyan;
