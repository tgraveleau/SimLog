...
/**
 * Classe G�rant un Tableau de Karnaugh
 */
...
public class SimLogKarnaugh {
	...
	// Tableau de Bool�en d�crivant l'expression logique
	...
	// Table des Correspondances entre la Table de Bool�ens et la Table de
	// Karnaugh

	private SimLogTableCorres TableCorres;

	// R�sultat du Regroupement
	...
	public SimLogKarnaugh(boolean[][] Table, int nv, int nm) {
		...
		// Cr�ation de la Table de Karnaugh
	...
	public void InitKarnaugh() {
		for (int i = 0; i < NbMonome; i++) {
			...
			for (int j = 0; j < NbVar * 2; j = j + 2) {
				// On D�termine la Valeur des Bits
				...
				if (Matrice[i][j] && Matrice[i][j + 1]) {
					// Si on a une Variable et sa N�gation
					// On met la Ligne � Vide pour ne pas la Prendre en Compte
			...
			// On Traite les Cases Vides Initialis�es � -1
			...
			if (vide == NbVar) {
				...
				// Calcule la Table de V�rit� des Cases � -1

				Verite = TableVerite(vide);

				// On Met � Jour la Matrice
	...
	// Initialise le Tableau de Bits � -1
	...
	// Pour Cr�er le Tableau de Bits Correspondant � l'Entier x
	...
	// Calcul la Table de V�rit� pour NbVar
	...
	// Renvoie un Tableau Contenant les Indices des Bits � -1
	...
	// Renvoie le Nombre de Cases � -1 dans T
	...
	// Remplit la Matrice dst � partir de la Matrice src
	// c1 : La Colonne � Copier
	// c2 : La Colonne � Modifier

	public void CopieColMat(int[][] src, int[][] dst, int c1, int c2,
			int NbLignes) {
	...
	// On Teste si le Masque M est un "Sous Masque" d'un Masque Accept� et
	// Stock� dans StockMask
	...
	public boolean EstIntersection(int intersec, int[] InterMask,
			int[][] StockMask, int NbMask) {
		if (NbMask > 0) {
			int i = 0;
			while (i < NbMask) {
				if (i != intersec) {
	...
	public int SuppInterMask(int[][] StockMask, int NbMask) {
		for (int i = 0; i < NbMask; i++) {
			int vide = NbCasesVides(IndicesCasesVides(StockMask[i], NbVar),
					NbVar);
			...
			// on teste si les cases du masque ne sont pas toutes incluses dans
			// les autres masques de StockMask
			boolean IntersectionRecouverte = true;
			int d = 0;
			while (IntersectionRecouverte && (d < NbLigCaseMask)) {
				if (!EstIntersection(i, CaseMask[d], StockMask, NbMask)) {
					IntersectionRecouverte = false;
				}
				d++;
			}
	...
	// On Initialise la Matrice qui Contient Toutes les Cases du Masque
	// (CaseMask)
	...
	// Fonction qui Teste si un Masque peut �tre Appliqu� � Partir de sa Table
	// CaseMask
	...
	public boolean ValidMask(int[][] CaseMask, int NbLigCaseMask) {
		...
		while (valid && (i < NbLigCaseMask)) {
			...
			valid = TableK[TableCorres.getNbLig(NumCase)][TableCorres
					.getNbCol(NumCase)];
			i++;
		}
	...
	// Renvoie le Tableau des Masques Accept�s ainsi que leur Nombre
	...
	public int[][] regroupement(int[][] StockMask, int NbMask) {
		...
		// Nombre de Masques Accept�s

		NbMask = 0;

		// D�finition des Variables pour la Table qui Contient les Cases d'un
		// Masque
		...
		if (ValidMask(CaseMask, NbLigCaseMask)) {
			...
			// On G�n�re Tous les Autres Masques
			// On G�n�re la Table de V�rit� qui Donne les Positions des Colonnes
			// non Vide

			int[][] Verite = TableVerite(NbVar);

			for (int i = 1; i < NbVar + 1; i++) {
				// Pour Chaque Bloc de Masques Ayant le m�me Nombre de -1

				int NbLigMask = (int) Math.pow(2, i);

				// On G�n�re une Petite Table de V�rit� Provisoire

				int[][] VeriteTemp = TableVerite(i);

				// Parcours de la Table de V�rit�
				...
				for (int l = 0; l < NbLigVerite; l++) {
					...
					if (NbUn(Verite[l]) == i) {
						...
						for (int c = 0; c < NbVar; c++) {
							if (Verite[l][c] == 1) {
								CopieColMat(VeriteTemp, Mask, ColVeriteTemp, c,
										NbLigMask);
								ColVeriteTemp++;
							}
						} // Fin Parcours de la Ligne dans V�rit�

						// Parcours de Mask

						for (int w = 0; w < NbLigMask; w++) {

							// On Teste si le Masque de la Ligne w est un Sous
							// Masque
							...
							if (!EstSousMask(Mask[w], StockMask, NbMask)) {
								...
								vide = NbCasesVides(
										IndicesCasesVides(Mask[w], NbVar),
										NbVar);

								// Nombre de Lignes dans CaseMask

								NbLigCaseMask = (int) Math.pow(2, vide);
								CaseMask = IniCaseMask(Mask[w], NbLigCaseMask,
										vide);
				...
				} // Fin Parcours des Lignes de V�rit�
			} // Fin Grand Bloc
		}

		// une fois tous les masques trouv�s, il faut enlever les masques qui
