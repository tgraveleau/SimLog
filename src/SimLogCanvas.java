...
public class SimLogCanvas extends JPanel
	implements MouseListener, MouseMotionListener, ActionListener {
		 ...
		 *  default constructor
		 *
		 *  @param win parent frame or application
		 */
		...
		public SimLogCanvas( SimLogWin win ) {
				...
				addMouseListener((MouseListener)popupListener);
		...
		public void paintComponent( Graphics g ) {
				...
				if (deleFlag == true) {
					...
					deleXPoly[0] = deleStartX; deleYPoly[0] = deleStartY;
					deleXPoly[1] = deleEndX;   deleYPoly[1] = deleStartY;
					deleXPoly[2] = deleEndX;   deleYPoly[2] = deleEndY;
					deleXPoly[3] = deleStartX; deleYPoly[3] = deleEndY;
					deleXPoly[4] = deleStartX; deleYPoly[4] = deleStartY;
				...
				if (intersectFlag == true) {
					intersectFlag = false;
					g.setColor( Color.red );
					g.drawRect( intersectXPos, intersectYPos, SimLogGate.WIDTH, SimLogGate.HEIGHT );
				}
		...
		private int addLink( ) {
				if (srcGateToLink == null) return -1;
				if (dstGateToLink == null) return -2;

				if (srcGateToLink == dstGateToLink) return -5;
				...
				default:
					...
					if (wind.getState() == true) {
						...
						if (circuit.gateHasAlreadyNameExcept( g, name )) {
							appli.messageWarning( "Gate with name " + name + " already exists" );
						} else {
		...
		/**
		 *  Mouse Clicked
		 *  Used when in GATE state to deposit a gate
		 *  or in DELEte state to remove gate under mouse
		 */
		...
		/**
		 *  check if circuit is valid, i.e. all input and output gates
		 *  are connected
		 *
		...
		/**
		 *  enter edition mode where you can move, remove or rename gates,
	   ...
	   *
		...
		public void actionPerformed( ActionEvent e ) {
			...
			if (e.getSource()==popupItems[0]) {
				int gateNbr = circuit.getGateIndexAtPos(popupListener.mouseX,popupListener.mouseY);
				if (gateNbr != -1) {
					int n;
					//gate = (SimLogGate) listOfGates.elementAt( gateNbr );
					circuit.removeGateAtIndex(gateNbr);
			...
			if (e.getSource()==popupItems[3]) {
				gate=circuit.getGateAtPos(popupListener.mouseX,popupListener.mouseY);
				if (gate!=null) {
					if ((gate.getType() >= SimLogGate.AND_GATE) && (gate.getType() <= SimLogGate.XOR_GATE)) {
						SimLogReplaceGateWin win = new SimLogReplaceGateWin( appli, gate, circuit );
						win.centerComponent();
						win.show();
						if (win.getState() == true) {
