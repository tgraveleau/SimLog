...
public class SimLogPlaCanvas extends JPanel
 ...
 // dimensions (arbitraires) d'une zone d'intersection pour la PlaInterface
  ...
  /**
	 *  Constructor
	 *
	 *  @param nv number of boolean variables
	 *  @param nm number of monomials
	 */
	...
	public SimLogPlaCanvas(SimLogPla parent, int nv , int nm)	{	
	  ...
	  //dimension des X : on multiplie par deux car on a deux colonnes par variables (var+négation)
 	 //				on rajoute var*16 car on affiche en bout de ligne le monome
	  //				au maximum on a "var" variables, et il faut 16 pixels pour afficher la lettre
  ...
  public void paintComponent(Graphics g) {
	  ...
	  for ( int i = 0 ; i < ( NbVar * 2 ) ; i++ ) {
		  // changement de couleur pour les négations
		  if ( ( i % 2 ) == 0 ) { 
    ...
    // on trace les négations au dessus des lettres
    g.setColor(Color.blue);
    Yaux = yZone - 12;    
    Xaux = xZone + (xZone / 2);
    ...
    for ( int i = 0 ; i < NbVar ; i++ ) {
	  ...
	  for ( int i = 0 ; i < NbVar * 2 ; i++ ) {
		  for ( int j = 0 ; j < NbMonome ; j++ ) {
			  if (TablePla[j][i]) {
			    g.fillOval((i*xZone+(xZone/2))-5,(j*yZone+2*yZone)-5,10,10); 
				}
			}
	 	}
	    ...
	    for ( int i = 0 ; i < ( NbVar * 2 ) ; i++ ) {
		    if (TablePla[j][i]) {
		      ...
		      if ( ( i % 2 ) == 1 ) {
    			...
    			// on décale l'indice Xaux pour la lettre suivante
					Xaux += 15;
		    }
		  } // fin pourX	
		  Xaux = NbVar * 2 * xZone;
  ...
  public void mouseClicked(MouseEvent e) {
    ...
    //les deux premieres zones sont pour la barre de la fenetre et les variables
    int divY = (coordY / yZone) - 2;
    // determination de la ligne de la matrice
    if ((coordY%yZone) > (yZone/2)) divY++; 
    // test de validité des coordonnées
    if ( ( divY >= 0 ) && ( divX <= NbVar * 2 ) && ( divY < NbMonome) ) {
			// vérification que l'on n'a pas une variable et sa négation sur la meme ligne
			if (( divX % 2 ) == 0 ) {		      
	      ...
	      // on veut la négation (indice impair)  
		    if ( TablePla[divY][divX - 1] == false ) {
   	  		TablePla[divY][divX] = !TablePla[divY][divX];
		      erreur = false;
		      repaint();
