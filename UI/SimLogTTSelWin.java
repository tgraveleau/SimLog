...
public class SimLogTTSelWin extends JDialog implements ActionListener {
	...
	private void lookForInputs(int tab[]) {
		...
		for (i = 0; i < tab.length; i++) {
			if (tab[i] == 1)
				lookForInputsForGate(tabOutputGates[i]);
		}
	}
	...
	class MyTableModel extends AbstractTableModel {
		final String[] columnNames = { "LED Name", "Select" };
		private Object[][] data;
		...
		public void setValueAt(Object value, int row, int col) {
			/*
			 * if (DEBUG) { System.out.println("Setting value at " + row + "," +
			 * col + " to " + value + " (an instance of " + value.getClass() +
			 * ")"); }
			 */
			if (data[0][col] instanceof Integer && !(value instanceof Integer)) {
				...
				try {
					...
					/*
					 * JOptionPane.showMessageDialog(appli, "The \"" +
					 * getColumnName(col) +
	...
	private void generate() {
		...
		for (i = 0; i < tabSelectedIndices.length; i++) {
			if (tabSelectedIndices[i] == 1)
				vOut.add(tabOutputGates[i]);
		}
