...
public class SimLogWin extends JFrame implements ActionListener {
	...
	private String menuLabels[] = { "Menu", "File", "New", "Load", "Save",
			"Quit", "Menu", "Action", "Edition", "Simulation", "Menu",
			"Display", "!1", "Gate name", "Grid", "Menu", "Tools", "Formula",
			"Karnaugh", "Nand/Nor", "Truth table", "Reorganize", "Menu", "?",
			"About", "Licence" };
	...
	/**
	 * Display a message to inform the user that a functionality is not
	 * implemented
	 */

	public void notImplemented() {
		JOptionPane w = new JOptionPane();
		JOptionPane.showMessageDialog(this, "This functionality is not yet implemented",
				"Not implemented", JOptionPane.WARNING_MESSAGE);
	}

	/**
	 * Display a message to inform the user that a fonctionality can not be used
	 * in applet mode
	 */

	public void notAllowedInAppletMode() {
		JOptionPane w = new JOptionPane();
		JOptionPane.showMessageDialog(this, "Not allowed in Applet mode", "Not Allowed",
	...
	public void newCircuit() {
		...
		if (win.getState() == true) {
			...
			for (i = 1; i <= n; i++) {
				circuit.addGate(20, 20 + (SimLogGate.HEIGHT + 10) * (i - 1),
						SimLogGate.SWITCH_GATE,
						(String) gateNames.elementAt(i - 1));
			}
			...
			for (i = 1; i <= n; i++) {
				circuit.addGate(200, 20 + (SimLogGate.HEIGHT + 10) * (i - 1),
						SimLogGate.LED_GATE,
						(String) gateNames.elementAt(i - 1));
			}
			gateNames.removeAllElements();
